...
pow:
    # Program dictionary:
    # r0, r1 - function arguments
    # r2 - current calculated product
    # r3 - exponent counter
    # r4 - temporary calculations
    ...
    # Initialize 
    MOV r2, r0	// current calculated product as base in r2
    MOV r3, #1	// exponent counter at r3 = 1 when r2 = base

    # if exponent (r1) == 0, return 1 
    # else: return result in r2
    CMP r1, #0
    MOVEQ r0, #1
        BEQ pow_done
	
    # if exponent (r1) == 1, return current calculated product (r2)
    # else: execute pow_loop which continues multiplying the base
	CMP r1, #1
	BEQ pow_done
	...
	pow_done:
	    # Move result to output and return result
	    MOV r0, r2	
...
#    suppose n = a x b. If both a and b are greater than sqrt(n), 
#      then a x b > n, which is not a valid case.
#    so if n has any divisors, at lease of of them must be less or 
#.     equal to sqrt(n)
# 
isPrime:
  # push the stack
  ...
  BLT NotPrime
    ...
    loop:
      ...
      BGT Prime   
        ...
        # if no remainder, the number is evenly divisible and the input 
        # is determined not prime
        MOV r0, r6
        MOV r1, r4
        BL __aeabi_idiv
        MOV r5, r0
...
cprivexp:
    ...
    #    calculate and update the inverses
    #    keep track of and update the remainder base and divisor 
    # else: 
    # 	 if remainder base != 1, no modular inverse, so return error
    #    else: branch to checkOldInverse
    startCprivexpLoop:
        CMP r4, #0 
    ...
    # if old inverse is negative, fix the negative inverse using totient
    # otherwise, return the old inverse 
    checkOldInverse:
        ...
        BGE returnOld_inverse 	@ No changes necessary, return old_inverse as is
            ...
            B endCprivexp 	@ return r0 (this is d)

    returnOld_inverse:
        MOV r0, r6 		@ r0 <- old_inverse
        B endCprivexp 		@ return old_inverse (this is d)
...
cpubexp:
  ...
  # Initialize
  MOV r4, #2 		@ Initialize e (r4 <- 2)
  MOV r5, r0 		@ r5 <- totientValue

  # while e < totient: 
  #   Validate e
  #     If e is valid, e++
  #   else: exit the loop as e is >= totient
  # else: return error as e is invalid
  startCpubexpLoop:
    CMP r4, r5
...
# Author: Alfredo Ormeno Zuniga
# Purpose:  Generates private and public keys
#
# Algorithm:
#   Display prompts to input two positive integers p and q. 
#	Check if both integers are positive, small numbers, and prime
#   Calculate the modulus n for the public and private keys: n = p * q
...
generateAndDisplayKeys:
	...
	startPLoop:
		...
		# Load the value p into preserved register
		LDR r4, =valueP
		LDR r4, [r4, #0]
...
encrypt:
    ...
    # Load scanned values from memory
    LDR r0, =inputString
    LDR r1, =publicExponent
    LDR r1, [r1, #0]
    LDR r2, =modulusN
